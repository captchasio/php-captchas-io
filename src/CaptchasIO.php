...
use CaptchasIO\Task\AbstractTask;
use GuzzleHttp\Client as GuzzleClient;
use Psr\Http\Message\ResponseInterface;
use Psr\Log\AbstractLogger;
use CaptchasIO\Service\AbstractService;
use CaptchasIO\Exception\CaptchasIOException;
use CaptchasIO\Exception\InvalidCaptchasIOServiceException;
...
class CaptchasIO
{
     ...
     * Constants list
     */
    const SERVICE_CaptchasIO = 'CaptchasIO';
    ...
    protected static $serviceMap = [
        self::SERVICE_CaptchasIO,
    ];

    /**
     * CaptchasIO constructor.
     * @param string|AbstractService $service
     * @param array $options
     *
     * @throws CaptchasIOException
     * @throws InvalidCaptchasIOServiceException
     */
    public function __construct($service, array $options = [])
    {
        if (is_string($service)) {
            if (false === in_array($service, self::$serviceMap)) {
                throw new InvalidCaptchasIOServiceException($service);
            }

            $serviceNamespace = '\\CaptchasIO\\Service\\' . $service;
    ...
    public function recognizeImage($image, $url = null, $params = [], $languagePool = 'en')
    {
        ...
        if (empty($taskId)) {
            throw new CaptchasIOException('Did not created task');
        }
    ...
    protected function getResult($taskId)
    {
        ...
        while (true) {
            ...
            if ($waitTime > $this->options['timeout_max']) {
                ...
                throw new CaptchasIOException('Timeout of resolving captcha');
            }
    ...
    protected function resolveResponse($body)
    {
        ...
        if ($data['errorId'] !== 0) {
            throw new CaptchasIOException($data['errorDescription'], $data['errorCode']);
        }
